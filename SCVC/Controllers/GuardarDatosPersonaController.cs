...
namespace SCVC.Controllers
{
    public class GuardarDatosPersonaController : Controller  {
        ...
        public async Task<IActionResult> GuardarPersona(PersonasViewModel persona)
        {
            ...
            else
            {
                try
                {
                    ...
                    await this.Api.Post<Persona>(DatosPersona, "https://apiscvc.azurewebsites.net/Persona/Post/", persona.Token);
        ...
        public async Task<IActionResult> GuardarVacuna(Vacunacion vacunas)
        {
            if(!ModelState.IsValid)
            {
                return BadRequest();
            ...
            else
            {
                try
                {
                    var VacunasG = new VacunasViewModel()
                    {
                        IdPersona = vacunas.IdPersona,
                        IdEnfermero = vacunas.IdEnfermero,
                        IdVacuna = vacunas.IdVacuna,
                        Dosis = vacunas.Dosis,
                        IdUsuario = vacunas.IdUsuario
                    };
                    var dato = await this.Api.Post<VacunasViewModel>(VacunasG, "https://apiscvc.azurewebsites.net/Vacunacion/Post/", vacunas.Token);
                    return RedirectToAction("MenuPrincipal", "Opciones");
                }catch(Exception Ex)
                {
                    return BadRequest($"Error: {Ex.Message}");
